    ...
    return (
      <nav className="relative z-10">
        <div className="container mx-auto px-4 flex items-center justify-between h-16">
          {/* Logo */}
          <div className="text-xl font-bold">Beak</div>
        ...
        <div
          className={cn(
            "md:hidden fixed top-0 left-0 h-full w-full bg-primary z-20 transform transition-transform duration-300 ease-in-out",
            isOpen ? "translate-x-0" : "-translate-x-full"
          )}
        >
          <div className="flex flex-col mt-16 space-y-4 px-4">
            {React.Children.map(children, (child) => (
              <div className="border-b border-secondary">{child}</div>
            ))}
          </div>
        </div>
      </nav>
...
>(({ className, ...props }, ref) => (
  <NavigationMenuPrimitive.Content
    ref={ref}
    className={cn(
      "left-0 top-0 w-full md:absolute md:w-auto data-[motion^=from-]:animate-in data-[motion^=to-]:animate-out data-[motion^=from-]:fade-in data-[motion^=to-]:fade-out",
      className
    )}
    {...props}
  />
